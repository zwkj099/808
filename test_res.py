            ...
            if abs(int(time.strftime("%H%M%S", time.localtime())) - t) >= pdict['period']:
                ...
                try:
                    ...
                    if id in ["8201", "8202","8802","8500"]:
                        ...
                        th = threading.Thread(target=attach_upload.attach_upload, args=(ip,tp, res, mobile, attach,filepath,pdict['version']))
                        th.start()
...
ip = "112.126.64.32"
port = 7003  # 6994川标,6995冀标，6975部标,6996桂标，6997苏标，6998浙标，6999吉标，7000陕标,7002沪标；7003中位标准；7006北京标准
deviceid =1911200 #20190928
mobile = 19110000024 #123456789
vnum = u"9622233"  #桂BB001
cont = 0
name = "驾1687"
...
attach = '02'  #00图片、02视频
filepath = './resourceFile/00_E1_E100_0_b87441129fb34ef8969fd0de2739d1322.mp4'
