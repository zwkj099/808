...
def test1(ip, port, mobile, deviceid, vnum, name, qualification,i=0,tal=0):
    ...
    3.wsid：主动安全报警附加信息
    """
    extrainfo_id = [20,21,22,23,24]#[1,48,49]  # [1,2,3,20,21,22,23,24,48,49]#传入需要组装的附件信息ID,不传表示无附加信息;1：里程，2：油量，3：速度，48：信号强度，49：卫星颗数，20：视频相关报警，21：视频信号丢失报警状态，22：视频信号遮挡报警状态，23：存储器故障报警状态，24：异常驾驶行为报警详细描述

   #上传wifi数据时，必须同时上传基站数据，上传基站数据，0200状态要为未定位
    idlist = [79,80]  # [34, 39, 65,69,79,80,81,83,112,128],传入需要组装的传感器ID，十进制数；33,34,35,36,37:温度；38,39,40,41:湿度；65,66,67,68:油量、液位；69,70:油耗；79:电量检测,80:终端检测；81:正反转；83:里程；84:蓝牙信标；112,113:载重；128,129:工时；8：基站数据；8、9：wifi数据
    wsid = []  # 上传的主动安全报警类型；225-231是中位主动安全数据，冀标只有100和101；0: 表示不带主动安全数据；100：驾驶辅助功能报警信息；101：驾驶员行为监测功能报警信息；112：激烈驾驶报警信息；102：轮胎状态监测报警信息；103：盲区监测报警信息；113：卫星定位系统报警信息；川冀标切换只需改端口；
                  # 81,82北京主动安全数据
    ...
    while True:
        ...
        while True:
            if abs(int(time.strftime("%H%M%S", time.localtime())) - t) >= pdict['period']:
                ...
                pdict['jin'] += 0.001
                pdict['wei'] += 0.001
                pdict['speed'] = random.randint(1,6)
...
#  ip="zoomwell.cn"
# port = 7006  # 6994川标,6995冀标，6975部标,6996桂标，6997苏标，6998浙标，6999吉标，7000陕标,7002沪标；7003中位标准；7006北京标准
# deviceid =9611000 #20190928
# mobile = 96110000000 #123456789
# vnum = u"9611000"  #桂BB001
# cont = 0
# name = "驾0687"
# qualification = 15603519463400389712

port = 6994  # 6994川标,6995冀标，6975部标,6996桂标，6997苏标，6998浙标，6999吉标，7000陕标,7002沪标；7003中位标准；7006北京标准
deviceid =9111200 #20190928
mobile = 91110000022 #123456789
vnum = u"9622222"  #桂BB001
cont = 0
name = "驾1687"
qualification = 15613519463400389712
