...
def attach_upload(ip,tp,res,mobile,attach,filepath,version):
    if version == 1:
        ...
        print "alarmflag",alarmflag
        alarm_number = res[68+iplenth:132+iplenth]
        print "alarm_number", alarm_number
        filename1 = os.path.basename(filepath)
        filename = tp.character_string(filename1)
        filename2 = tp.character_string1(filename1,100)
        #filesize文件大小，filesize1文件大小转为16进制
        ...
        usual_body = jctool.get_id(deviceid) + alarmflag + alarm_number + '00' + '01' + attachlist  # 组装1210指令消息体
        head = tp.data_head(mobile, 4624, usual_body, 5, version)
        redata = tp.add_all(head + usual_body)
