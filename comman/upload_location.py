...
def location(tp,link,mobile,pdict,ex808dict,sensordict,info,extrainfo_id,idlist,wsid,answer_number=0000):
    ...
    zds,extrainfos,oils,wds,sds,yhs,zfs,zzs,gss,lcs,base,wifi,lys = info
    gpsdata = tp.position(mobile, pdict['messageid'], 6, 1, pdict['alarm'], pdict['status'], pdict['jin'], pdict['wei'],\
                          ...
                          tp.extra_info(extrainfo_id,extrainfos),tp.zd_body(wsid,zds),tp.f3_attach(idlist,oils,wds,sds,yhs,zfs,zzs,gss,lcs,base,wifi,lys), pdict['version'],answer_number)
