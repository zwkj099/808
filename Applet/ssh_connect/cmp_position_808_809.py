...
def cmp_pos(host,user,pwd,simcard,vnum):
    """
    :param host: ssh服务器ip
    :param user: 登录名
    :param pwd: 密码
    ...
    while (res_809=='' or (int(rec_time[:-4].replace(":",""))<int(send_time.replace(":",""))) or result1.find('gps')==-1)and j<30: ##有时候接收到的json没有gps
        stdin, stdout, stderr = ssh.exec_command(
            "tail -n 100 /home/f3/809/info.log |grep '%s'"%vnum)  # 执行命令并获取命令结果
    ...
    start1 =[m.start() for m in re.finditer('{"data":{"msgBody"',result1)][-1]
    data_809 =result1[start1:]
    data_809 =json.loads(data_809) #字符串转为字典形式
    # print data_809
    m= data_809['data']["msgBody"]["data"]["gps"]
    rec_data=[m["alarm"],m["state"],m["lat"],m["lon"],m["altitude"],m["vec1"],m["direction"],str(m["year"])[2:],m["month"],\
              m["day"],m["hour"],m["minute"],m["second"],m["vec3"],m["vec2"]]
