...
def main(browser,args,testinfo,tp,link,app,mobile,vnum,logfile,logtofile):
    ...
    for key,value in info.items():
        #轮询下发
        sensor_data = app.set_obd_parameter(value,browser,vnum,logfile,logtofile)
        time.sleep(8)
        
        app.send_obd_parameter(browser,logfile,logtofile)
...
#                 time.sleep(5) 
        ...
        for res in reslist:
            result = app.obd_compare(sensor_data,res,mobile,logfile,logtofile)
            if result==True:
#                 win32api.MessageBox(0, "polling test pass", "OK",win32con.MB_OK)
                print "OBD test pass"
                logtofile(logfile,"OBD test pass\n")
            else:
                logtofile(logfile,"OBD test fail\n")
...
def readconfig(sensordict):
    ...
    bfail = False
    if os.path.exists("./Automation/AppManager/appconfig/obd.xml") == True:
        try:
            dom = minidom.parse("./Automation/AppManager/appconfig/obd.xml")
