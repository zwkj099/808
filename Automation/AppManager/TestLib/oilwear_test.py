...
#         [noLoadValue,noLoadThreshold,lightLoadValue,lightLoadThreshold,fullLoadValue,fullLoadThreshold,overLoadValue,overLoadThreshold]=sensor_data
        sensor_data = app.set_oilwear_parameter(value,browser,vnum,logfile,logtofile)
        time.sleep(8)
        
        app.send_oilwear_parameter(browser,logfile,logtofile)
...
#                 time.sleep(5) 
        ...
        for res in reslist:
            result = app.oilwear_compare(sensor_data,res,mobile,logfile,logtofile)
            if result==True:
#                 win32api.MessageBox(0, "polling test pass", "OK",win32con.MB_OK)
                print "oil wear test pass"
                logtofile(logfile,"oil wear test pass\n")
            else:
                logtofile(logfile,"oil wear test fail\n")
#                 win32api.MessageBox(0, "oil wear test fail", "Fail",win32con.MB_ICONWARNING)
                print "oil wear test fail"
#     browser.quit()

def readconfig(sensordict):
    from xml.dom import minidom
    bfail = False
    if os.path.exists("./Automation/AppManager/appconfig/oilwear.xml") == True:
        try:
            dom = minidom.parse("./Automation/AppManager/appconfig/oilwear.xml")
