        ...
        try:    
            for res in reslist:
                ...
                if res[2:6]=="8103":#解析数据
                    ...
                    #F3_id 外设id 数据长度
                    #起始位置到终止位置，hex表示不转换，float表示转为浮点数，否则转为十进制
...
def readconfig(sensordict):
    ...
    bfail = False
    if os.path.exists("./Automation/AppManager/appconfig/load.xml") == True:
        try:
            dom = minidom.parse("./Automation/AppManager/appconfig/load.xml")
