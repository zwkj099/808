...
def main(browser,args,testinfo,tp,link,app,mobile,vnum,logfile,logtofile):
    ...
    for key,value in info.items():
        #轮询下发
        sensor_data = app.set_temp_parameter(value,browser,vnum,logfile,logtofile)
        time.sleep(8)
        
        app.send_temp_parameter(browser,logfile,logtofile)
...
#                 time.sleep(5) 
        ...
        for res in reslist:
            result = app.temp_compare(sensor_data,res,mobile,logfile,logtofile)
            if result==True:
#                 win32api.MessageBox(0, "polling test pass", "OK",win32con.MB_OK)
                print "temp test pass"
                logtofile(logfile,"temp test pass\n")
            else:
#                 win32api.MessageBox(0, "oil wear test fail", "Fail",win32con.MB_ICONWARNING)
                print "temp test fail"
#     browser.quit()

def readconfig(sensordict):
    from xml.dom import minidom
    bfail = False
    if os.path.exists("./Automation/AppManager/appconfig/temp.xml") == True:
        try:
            dom = minidom.parse("./Automation/AppManager/appconfig/temp.xml")
