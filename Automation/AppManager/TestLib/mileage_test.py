...
def main(browser,args,testinfo,tp,link,app,mobile,vnum,logfile,logtofile):
    ...
    for key,value in info.items():
        #轮询下发
#         time.sleep(5)
        sensor_data = app.set_mileage_parameter(value,browser,vnum,logfile,logtofile)
        time.sleep(8)
        
        app.send_mileage_parameter(browser,logfile,logtofile)
...
#                 time.sleep(5) 
        ...
        for res in reslist:
            result = app.mileage_compare(sensor_data,res,mobile,logfile,logtofile)
            if result==True:
#                 win32api.MessageBox(0, "polling test pass", "OK",win32con.MB_OK)
                print "mileage test pass"
                logtofile(logfile,"mileage test pass\n")
            else:
                logtofile(logfile,"mileage test fail\n")
#                 win32api.MessageBox(0, "oil wear test fail", "Fail",win32con.MB_ICONWARNING)
                print "mileage test fail"
#     browser.quit()

def readconfig(sensordict):
    from xml.dom import minidom
    bfail = False
    if os.path.exists("./Automation/AppManager/appconfig/mileage.xml") == True:
        try:
            dom = minidom.parse("./Automation/AppManager/appconfig/mileage.xml")
            ...
            for snpninfo in supportpdtlst:
                ...
                tyreSizeId = snpninfo.getElementsByTagName("tyreSizeId")[0].childNodes[0].data
  
                           
                sensordict[1] = (autouploadtime,output_k,output_b,tyreSizeId)
